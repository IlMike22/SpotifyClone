...
import de.mindmarket.spotifyclone.other.Event
...
class MusicServiceConnection(
    ...
    val transportControls: MediaControllerCompat.TransportControls
        get() = mediaController.transportControls // written so because we do not have access to mediaController yet
            ...
            when (event) {
                NETWORK_ERROR -> {
                    _networkError.postValue(
                        Event(
                            Resource.error(
                                "Couldnt connect to server. Please check your internet connection.",
                                null
                            )
                        )
                    )
